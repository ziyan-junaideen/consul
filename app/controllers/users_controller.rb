  ...
  private
    ...
    def set_activity_counts
      @activity_counts = ActiveSupport::HashWithIndifferentAccess.new(
                          ...
                          budget_investments: (Setting["process.budgets"] ? Budget::Investment.project.where(author_id: @user.id).count : 0),
                          comments: only_active_commentables.count,
                          follows: @user.follows.map(&:followable).compact.count,
                          budget_ideas: (Setting['process.ideas'] ? Budget::Investment.idea.published.where(author_id: @user.id).count : 0))
    end
